  ...
  const items: MenuProps["items"] = [
    {
      ...
      label: (
        <div>
          <div className="text-xs text-secondary mb-1">{t("playground.selectTeam")}</div>
          <Input
            prefix={<TextSearch className="w-4 h-4" />}
            placeholder={t("playground.searchTeams")}
    ...
    <div className="space-y-2 w-full">
      ...
      >
        <div className="" style={{ width: "183px" }}>
          <Plus className="w-4 h-4 inline-block -mt-1" /> {t("playground.newSession")}
        </div>
      </Dropdown.Button>
      ...
      {hasNoTeams && (
        <div className="flex items-center gap-1.5 text-xs text-yellow-600 mt-1">
          ...
          <span>{t("playground.createTeam")}</span>
        </div>
      )}
    </div>
  );
