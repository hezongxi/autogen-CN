  ...
  const items: MenuProps["items"] = [
    {
      ...
      label: (
        <div>
          <div className="text-xs text-secondary mb-1">{t("playground.searchSessions")}</div>
          <Input
            prefix={<TextSearch className="w-4 h-4" />}
            placeholder={t("playground.searchSessions")}
      ...
      >
        <Tooltip title={session?.name}>
          <span className="text-primary font-medium truncate overflow-hidden">
            {session?.name || t("playground.selectSession")}
          </span>
        </Tooltip>
