...
export const UISettingsPanel: React.FC<UISettingsPanelProps> = ({ userId }) => {
  ...
  const handleReset = async () => {
    try {
      ...
      if (!serverSettings) {
        messageApi.error(t('settings.settingsNotLoaded'));
        setIsSaving(false);
        return;
      }
      ...
      setIsDirty(false);
      messageApi.success(t('settings.settingsResetSuccess'));
    } catch (error) {
      console.error("Failed to reset UI settings:", error);
      messageApi.error(t('settings.settingsResetFailed'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      setIsDirty(false);
      messageApi.success(t('settings.settingsSaveSuccess'));
    } catch (error) {
      console.error("Failed to save UI settings:", error);
      messageApi.error(t('settings.settingsSaveFailed'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className=" ">
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-medium">{t('settings.uiSettings')}</h3>
        <div className="space-x-2 inline-flex">
          <Tooltip title={t('settings.resetToDefaults')}>
            <Button
          ...
          <Tooltip title={isDirty ? t('settings.saveChanges') : t('settings.noUnsavedChanges')}>
            <Button
              type="primary"
              icon={
                <div className="relative">
      ...
      <div className="space-y-0 rounded border border-secondary">
        <SettingToggle
          ...
          label={t('settings.showLLMEvents')}
          description={t('settings.showLLMEventsDesc')}
          disabled={isSaving}
        />
        ...
        <SettingToggle
          ...
          label={t('settings.expandMessagesByDefault')}
          description={t('settings.expandMessagesByDefaultDesc')}
          disabled={isSaving}
        />
        ...
        <SettingToggle
          ...
          label={t('settings.showAgentFlowByDefault')}
          description={t('settings.showAgentFlowByDefaultDesc')}
          disabled={isSaving}
        />
      </div>
