...
> = ({ serverSettings, loading, userId, initializeSettings }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (!serverSettings) {
        messageApi.error(t("environmentVariables.settingsNotLoaded"));
        return;
      }
  ...
  const columns: ColumnTypes[] = [
    {
      title: t("environmentVariables.name"),
      dataIndex: "name",
      width: "55%",
      editable: true,
      inputType: "text",
    ...
    {
      title: t("environmentVariables.value"),
      dataIndex: "value",
      width: "45%",
      editable: true,
      inputType: "password",
    ...
    {
      title: t("environmentVariables.actions"),
      dataIndex: "operation",
      render: (_, record) => (
        <Tooltip title={t("environmentVariables.deleteVariable")}>
          <Button
            type="text"
            danger
  ...
  return (
    <Card className="shadow-sm">
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-medium">{t("environmentVariables.title")}</h3>
        <div className="space-x-2 inline-flex">
          <Tooltip title={t("environmentVariables.addVariable")}>
            <Button
              type="default"
              icon={<Plus className="w-4 h-4" />}
              onClick={handleAdd}
            >
              {t("environmentVariables.add")}
            </Button>
          </Tooltip>
          <Tooltip title={isDirty ? t("environmentVariables.saveChanges") : t("environmentVariables.noUnsavedChanges")}>
            <Button
              type="primary"
              icon={
