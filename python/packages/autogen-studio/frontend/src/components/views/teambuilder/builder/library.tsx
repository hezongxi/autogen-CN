...
export const ComponentLibrary: React.FC<LibraryProps> = ({
  ...
  const sections = React.useMemo(
    () => [
      ...
      {
        title: t("teamBuilder.terminations"),
  ...
  if (isMinimized) {
    return (
      ...
      >
        <span>{t('teamBuilder.showComponentLibrary')}</span>
        <button
          onClick={() => setIsMinimized(false)}
          className="p-1 group-hover:bg-tertiary rounded transition-colors"
          title={t('teamBuilder.maximizeLibrary')}
        >
    ...
    >
      <div className="rounded p-2 pt-2">
        <div className="flex justify-between items-center mb-2">
          <div className="text-normal">{t('teamBuilder.componentLibrary')}</div>
          <button
            onClick={() => setIsMinimized(true)}
            className="p-1 hover:bg-tertiary rounded transition-colors"
            title={t('teamBuilder.minimizeLibrary')}
          >
        ...
        <div className="mb-4 text-secondary">
          {t('teamBuilder.dragComponentInstruction')}
        </div>

        <div className="flex items-center gap-2 mb-4">
          <Input
            placeholder={t('teamBuilder.searchComponents')}
