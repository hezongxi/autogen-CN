    ...
    def upsert(self, model: BaseDBModel, return_json: bool = True) -> Response:
        ...
        with Session(self.engine) as session:
            try:
                ...
                if existing_model:
                    # Get current values and update with new values
                    current_values = existing_model.model_dump()
                    new_values = model.model_dump()
                    current_values.update(new_values)
                    current_values['updated_at'] = datetime.now()
                    
                    # Ensure datetime fields are proper datetime objects for SQLite
                    for dt_field in ['created_at', 'updated_at']:
                        if dt_field in current_values and isinstance(current_values[dt_field], str):
                            try:
                                current_values[dt_field] = datetime.fromisoformat(
                                    current_values[dt_field].replace("Z", "+00:00")
                                )
                            except ValueError:
                                # If parsing fails, use current time
                                current_values[dt_field] = datetime.now()
                    
                    # Create new validated instance to ensure all field validators are applied
                    validated_model = model_class(**current_values)
                    
                    # Transfer the validated values
                    for key, value in validated_model.model_dump().items():
