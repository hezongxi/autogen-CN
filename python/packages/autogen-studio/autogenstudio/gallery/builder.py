...
def create_default_gallery() -> GalleryConfig:
    ...
    builder.add_team(
        ...
        description="RoundRobinGroupChat团队中的单个AssistantAgent（带计算器工具）。",
    )

    critic_agent = AssistantAgent(
        name="critic_agent",
        system_message="你是个乐于助人的助手。批评助理的产出并提出改进建议。",
        description="对助理的产出进行批评和改进的代理人",
        model_client=base_model,
    )
    ...
    builder.add_team(
        ...
        label="Selector Team",
        description="一个由两名客服组成的团队——SelectorGroupChat团队中的一名助理客服（带计算器工具）和一名CriticAgent。",
    )
